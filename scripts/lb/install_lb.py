...
run('sudo bash -c "printf \'# ipvsadm configuration\n\n\' > /etc/default/ipvsadm"')
run('sudo bash -c "printf \'# if you want to start ipvsadm on boot set this to true\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'AUTO=\\"true\\"\n\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'# daemon method (none|master|backup)\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'DAEMON=\\"master\\"\n\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'# use interface (eth0,eth1...)\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'IFACE=\\"eth0\\"\n\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'# syncid to use\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'SYNCID=\\"1\\"\n\n\' >> /etc/default/ipvsadm"')
run('sudo bash -c "printf \'auto eth0:0\n\' >> /etc/default/ipvsadm"')
